...
function withoutBlockedSources(opportunities: Opportunity[]): Opportunity[] {
  const domains = readSourceBlocklist();
  if (domains.length === 0) return opportunities;
  return opportunities.filter((opportunity) => !opportunity.items.some((item) => isBlockedSourceURL(item.url, domains)));
}
...
export function TodayResearch({ loadToday = getTodayData, mutateDisposition = updateOpportunityDisposition }: TodayResearchProps) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setState({ status: "ready", data: { ...data, opportunities: withoutBlockedSources(data.opportunities) } });
    } catch (error) {
